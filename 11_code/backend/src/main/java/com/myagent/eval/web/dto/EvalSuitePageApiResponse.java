package com.myagent.eval.web.dto;

import com.myagent.common.api.ApiError;
import com.myagent.common.api.PageResponse;
import com.myagent.eval.application.result.EvalSuiteListItemResult;
import io.swagger.v3.oas.annotations.media.Schema;

/**
 * 验收套件列表统一响应。
 *
 * @param success 成功标记
 * @param data 分页数据
 * @param error 错误对象
 */
@Schema(name = "EvalSuitePageApiResponse", description = "验收套件列表统一响应。")
public record EvalSuitePageApiResponse(boolean success, PageResponse<EvalSuiteListItemResult> data, ApiError error) {
}
